...
const copy = {
  en: {
    ...
    intro: "These terms govern your access to and use of MlloMail. By using the service, you agree to these terms.",
    sections: [
      ["Service", "MlloMail provides temporary email addresses, message handling, sharing, and related account features. Features, retention periods, limits, and availability may change as the service evolves."],
      ["Acceptable use", "You must use MlloMail lawfully. You may not use the service to send spam, impersonate others, violate privacy or intellectual-property rights, distribute malware, evade platform safeguards, disrupt systems, or facilitate fraud or abuse."],
      ...
      ["Temporary data", "Temporary addresses and messages may expire or be deleted according to service settings. MlloMail is not an archival or backup service. You are responsible for saving any information you need to retain."],
      ...
      ["Disclaimer and liability", "To the extent permitted by applicable law, the service is provided without warranties of uninterrupted operation or fitness for a particular purpose. MlloMail is not responsible for indirect losses resulting from service interruption, message delivery failure, or loss of temporary data."],
  ...
  "zh-CN": {
    title: "服务条款",
    intro: "本条款适用于您对 MlloMail 的访问和使用。使用本服务即表示您同意遵守本条款。",
    sections: [
      ["服务内容", "MlloMail 提供临时邮箱地址、邮件处理、分享和相关账户功能。随着服务演进，功能、保留期限、使用限制和可用性可能发生变化。"],
      ["合理使用", "您必须合法使用 MlloMail。不得利用本服务发送垃圾信息、冒充他人、侵犯隐私或知识产权、传播恶意软件、规避平台保护措施、破坏系统，或协助欺诈及其他滥用行为。"],
      ["账户责任", "您应对自己账户下的活动负责，并妥善保护登录服务商的访问权限。您应提供准确信息；如怀疑账户被未授权使用，应及时通知服务运营方。"],
      ["临时数据", "临时地址和邮件可能根据服务设置过期或删除。MlloMail 不是归档或备份服务；您有责任保存需要长期保留的信息。"],
      ["服务可用性", "本服务按实际可用状态提供。为保障安全性、可靠性、法律合规或持续运营，我们可能修改、暂停、限制或停止部分功能。"],
      ["免责声明与责任限制", "在适用法律允许的范围内，我们不保证服务永不中断或适用于特定目的。对于服务中断、邮件投递失败或临时数据丢失造成的间接损失，MlloMail 不承担责任。"],
