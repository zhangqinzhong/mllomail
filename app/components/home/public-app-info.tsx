...
const content = {
  en: {
    ...
    title: "About MlloMail",
    description:
      "MlloMail is a temporary email service that helps users protect their primary inbox. Visitors can learn about the service on this page without creating an account or signing in.",
    features: [
      ...
      ["Google Sign-In", "Google Sign-In is optional and is used only to authenticate an account. MlloMail receives basic profile and email information and does not access Gmail, Google Drive, contacts, or calendars."],
    ],
  ...
  zh: {
    ...
    title: "关于 MlloMail",
    description:
      "MlloMail 是一项临时邮箱服务，帮助用户保护常用邮箱地址。访客无需创建账户或登录，即可在本页面了解服务用途和主要功能。",
    features: [
      ["临时邮箱", "创建一次性邮箱地址并接收邮件，无需暴露您的常用邮箱。"],
      ["注重隐私", "通过自动过期和邮箱分享控制，管理临时邮件和访问权限。"],
      ["Google 登录", "Google 登录是可选的，仅用于账户身份验证。MlloMail 只接收基本个人资料和邮箱信息，不会访问 Gmail、Google Drive、通讯录或日历。"],
    ],
    privacy: "隐私政策",
    terms: "服务条款",
  },
...
export function PublicAppInfo({ locale }: { locale: Locale }) {
  ...
  return (
    <section id="about" aria-labelledby="about-mllomail" className="pb-16 sm:pb-20">
      <div className="mx-auto max-w-5xl rounded-2xl border bg-background/90 p-6 shadow-sm sm:p-10">
        ...
        <h2 id="about-mllomail" className="mt-2 text-3xl font-bold tracking-tight">
          {copy.title}
        </h2>
